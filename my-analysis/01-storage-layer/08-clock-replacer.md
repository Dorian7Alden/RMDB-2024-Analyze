...
1. **分区小，扫描成本低**：每个 Instance 只有 4096 个 frame，victim 最多扫 8192 次（两圈），纯数组操作很快
2. **零动态分配**：数组大小编译期确定（`BUFFER_POOL_INSTANCE_SIZE`），不涉及 `new`/`delete`
3. **无内部锁**：Instance 已有 `latch_` 保护，Clock 不需要自己加锁，减少锁开销
