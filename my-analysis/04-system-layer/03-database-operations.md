...
> **注意**：`drop_db` 不需要先 `close_db`。它不关心数据库是否被打开，直接物理删除目录。如果在打开状态下被删除，`SmManager` 中的 `fhs_` 和 `ihs_` 句柄会变成悬空指针——因此正常使用中应该先 `close_db` 再 `drop_db`，或者 `drop_db` 后不再使用当前 SmManager。
