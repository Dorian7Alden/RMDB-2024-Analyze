...
> **`0x80` 是什么意思？**
>
> `0x` 前缀表示十六进制。`0x80 = 8×16 + 0 = 128`（十进制），写成二进制就是 `0b10000000`，最高位是 1。
>
> 为什么要用十六进制？因为位运算时关心二进制长什么样：
>
> | 十六进制 | 二进制 | 十进制 |
> |---------|--------|--------|
> | `0x80` | `10000000` | 128 |
...
> | `0x10` | `00010000` | 16 |
>
> 十六进制每一位对应 4 个 bit，`0x80` 一眼看出最高位是 1，比写 128 直观。
...
`char[] bitmap` 是一个字节数组。要操作第 `pos` 位，需要两样信息：
- **在哪个字节里？** → `get_bucket` 返回数组下标
- **在这个字节的哪个 bit 上？** → `get_bit` 返回一个"只有那一位是 1"的掩码

```cpp
static int get_bucket(int pos) {
  return pos / BITMAP_WIDTH;   // pos / 8
}

static char get_bit(int pos) {
  return BITMAP_HIGHEST_BIT >> (pos % BITMAP_WIDTH);
  //    0x80 = 0b10000000        pos % 8 = 余数 0~7
}
```

以 `pos = 10` 为例，对照 `char[] bitmap` 数组来看：
...
│  pos: 0  1  2  3  4  5  6  7  │  pos: 8  9 10 11 12 13 14 15 │
└───────────────────────────────┴──────────────────────────────┴─────
```

- `get_bucket(10)` = `10 / 8` = **1** → `bitmap[1]`，即第 1 号字节（第二个字节）
- `get_bit(10)` = `10 % 8` = **2** → 在这个字节里是第 2 位

**为什么右移可以获取到对应的 bit？**

`get_bit` 的任务是生成一个"掩码"——只有目标位置是 1，其余全是 0。将来用这个掩码去做 `|`（置 1）或 `&`（检测）。

`0x80 = 0b10000000`，这个 1 在**最左边**（bit 7）。右移就是把这个 1 往右推：

```
pos % 8 = 0:  0b10000000 >> 0  =  0b10000000   ← 1 在 bit 7，对应 slot 0
pos % 8 = 1:  0b10000000 >> 1  =  0b01000000   ← 1 在 bit 6，对应 slot 1
pos % 8 = 2:  0b10000000 >> 2  =  0b00100000   ← 1 在 bit 5，对应 slot 2 (pos=10 的情况)
...
pos % 8 = 7:  0b10000000 >> 7  =  0b00000001   ← 1 在 bit 0，对应 slot 7
```

> 想象一个台球在最左边的球袋，每右移 1 位，球就往右滚一格。
> `pos % 8` 就是要滚几格。滚完后球停在哪里，掩码的 1 就在哪里。

所以 `pos=10` 时：`get_bucket` 找到 `bitmap[1]`，`get_bit` 生成掩码 `0b00100000`。二者配合，精确锁定 `bitmap[1]` 的第 5 位（bit 5 = slot 2）。
...
> **`max_n` 传的是记录数，`i` 遍历的是 bit 位，对得上吗？**
>
> 对得上。`is_set(bm, i)` 的参数 `i` 是 **bit 下标**，不是字节下标。`is_set` 内部会调用 `get_bucket(i)` 把 bit 下标转成字节下标。
>
> 每个槽位恰好用 1 个 bit 标记，所以 bit 位数 = 槽位数 = `num_records_per_page`。循环 `i` 从 0 到 `max_n - 1`，正好遍历每个槽位对应的那个 bit。不用担心 `×8`——那是在把 bit 下标误解成字节下标时才会出现的想法。
