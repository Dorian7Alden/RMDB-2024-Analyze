...
> **为什么不能直接修复损坏的索引，非要重建？**
>
> 可以修，但不值得。B+ 树的节点之间高度相互依赖——内部节点里存着孩子页的页号，叶节点之间有链表指针：
>
> ```
> 内部节点 A: [key=20 → 孩子页3] [key=50 → 孩子页5]
>                                        ↓
> 叶节点 5: [key=50 → Rid{...}]  ← 假设这页损坏
> ```
>
> 如果叶节点 5 损坏，不只是丢几条索引条目——内部节点 A 还指着页 5，前后叶节点还通过链表指针连着它。你无法确认损坏范围：是一个 key 损坏了？还是一整页？还是链表指针断了？没有精确的损坏诊断，就不存在精确的修复。
>
> | | 修复 | 重建 |
> |---|------|------|
> | 前提 | 必须知道哪页错了、什么错误 | 只需要表数据是完整的 |
> | 实现 | 要为每种损坏模式写修复逻辑 | 全表扫描 + B+ 树插入，现成代码 |
> | 风险 | 修错了→索引与数据不一致→查询结果错误 | 只要表数据对，索引一定对 |
>
> 表数据（`.db` 文件）还在，全表扫描重插一遍 B+ 树，结果 100% 正确。这是典型的"重建比修复便宜"场景。
