...
**不能直接拿 int 和记录里的字节比较吗？然后全部用 memcmp？**

这两个问题其实指向同一件事：执行层到底怎么比较两个值。

理论上可以全部用 `memcmp` 逐字节比较。但**对于 FLOAT，这行不通**。

原因在于 IEEE 754 浮点数的二进制格式。FLOAT 类型的 `-1.0` 在内存里是 `0xBF800000`，`1.0` 是 `0x3F800000`。如果直接 `memcmp`，会得到 `0xBF800000 > 0x3F800000`，即 `-1.0 > 1.0`——完全错误。因为 IEEE 754 的最高位是符号位，负数最高位为 1，在字节层面看起来反而比正数"大"。

所以 RMDB 实际的做法是**类型感知的比较**，源码在 `src/execution/execution_defs.h:62-80`：
...
static inline int compare(const char* a, const char* b, int col_len,
    ...
    switch (col_type) {
        case TYPE_INT:
            return (*(int*)a > *(int*)b) - (*(int*)a < *(int*)b);    // 按 int 比
        case TYPE_FLOAT:
            return (*(float*)a > *(float*)b) - (*(float*)a < *(float*)b); // 按 float 比
        case TYPE_STRING:
            return memcmp(a, b, col_len);  // 逐字节比
    }
}
```

**含义**：执行层把记录中的 4 个字节重新解释为 `int` 或 `float`，用 C++ 的类型化比较运算符得出正确结果。STRING 则直接用 `memcmp`，因为 ASCII 字符串的字节顺序就是字典序。

**那 raw 的意义在哪**：

虽然执行层做的是类型化比较，但 Value 的 `raw` 仍然有意义。`compare()` 的两边都是 `const char*`——都要求值是字节形态。`raw` 把 `int_val=18` 转成 4 字节，`compare()` 再把它重新解释为 int 来比较。两边的格式统一了，比较函数才能用同一套接口处理所有类型。
