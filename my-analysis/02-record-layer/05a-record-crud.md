...
> **记录副本会不会和页面数据不一致？** 有可能。`get_record` 做的是"快照式"读取——`memcpy` 拷贝的是读取那一刻的页面数据。拷贝完成后，如果另一个事务修改了页面中同一个槽位，记录副本就和页面不一致了。但这不是 bug——保证数据一致性是上层**事务层**（锁管理器、隔离级别）的职责，不在记录层的范围内。记录层只负责"按地址搬运字节"，不管并发控制。
