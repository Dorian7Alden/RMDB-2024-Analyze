...
回顾 [04](./04a-buffer-pool-overview.md) 的 fetch_page 流程和 [05](./04b-buffer-pool-single.md) 的源码实现，Replacer 在其中扮演三个角色。以下是 BufferPool 调用 Replacer 的时机和触发条件：
...
- LRU 的链表操作和动态分配在分区小池子场景下成为瓶颈

LRU 与 Clock 的详细对比分析见：[05c. LRU 与 Clock 对比分析](./05c-replacer-comparison.md)


上一节：[04e-buffer-pool-steal-force.md](./04e-buffer-pool-steal-force.md)

下一节：[05b. Clock 时钟替换算法](./05b-replacer-clock.md) — 参考实现使用的近似 LRU

后续：[05c. LRU 与 Clock 对比分析](./05c-replacer-comparison.md)
