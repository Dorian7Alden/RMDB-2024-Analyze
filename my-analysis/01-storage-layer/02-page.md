...
- **帧（Frame）**：缓冲池中的"槽位"，一个帧对应一个 Page 对象。帧是内存概念，页是逻辑概念。（`common/config.h:48-49`）

## 数据结构

### PageId

页面由"哪个文件 + 文件中第几页"共同定位（`page.h:19-33`）：
...
下一节：[03. 缓冲池概述](./03-buffer-pool-overview.md)
