...
```
student.db 文件（4 页 × 4096 = 16384 字节）

┌──────────────────────────────────────────────────────────────────┐
│                       第 0 页（文件头）                            │
│                                                                  │
│  RmFileHdr {                                                     │
│    record_size          = 28     ← 每条记录 28 字节                │
│    num_pages            = 4      ← 当前共 4 页                    │
│    num_records_per_page = 140    ← 每页最多 140 条                │
│    first_free_page_no   = 3      ← 第 3 页有空位                  │
│    bitmap_size          = 18     ← 每页位图 18 字节               │
│  }                                                               │
│                                                                  │
│  ┌────────────────────────── 4076 字节空白 ────────────────────┐  │
│  │  文件头只用了几十字节，剩余空间闲置不用                         │  │
...
├──────────────────────────────────────────────────────────────────┤
│                       第 1 页（数据页，已满）                       │
│  140 条记录（id=1 ~ id=140 的 student 记录）                       │
├──────────────────────────────────────────────────────────────────┤
│                       第 2 页（数据页，已满）                       │
│  140 条记录（id=141 ~ id=280 的 student 记录）                     │
├──────────────────────────────────────────────────────────────────┤
│                       第 3 页（数据页，有空位）                     │
│  60 条记录（id=281 ~ id=340），还有 80 个空位                      │
└──────────────────────────────────────────────────────────────────┘
```
...
0       4       LSN（日志序列号，恢复用）
4       8       RmPageHdr {
                  next_free_page_no = 2   ← 此页满了，下一有空位的是第 2 页？不对，第 2 页也满了，应该是 3
...
第 1 页的内存布局图：
 ...
 ┌────┬──────┬────────────────────┬──────────────────────────────────────────┐
 │LSN │PageHdr│      Bitmap        │              记录数据区                  │
 │4B  │  8B   │      18B           │            3920B (28B × 140)            │
 ├────┴──────┴────────────────────┼──────────────────────────────────────────┤
 │ offset 30                      │ offset 48     offset 76                  │
 │ ┌──────────────────────────────┐│──────────────┬──────────────────────────┤
 │ │ id:1, name:"Alice", age:20  ││ id:2,         │ id:3, ...                │
 │ │                              ││ name:"Bob",   │                          │
 │ │ 28 字节                      ││ age:22        │                          │
 │ └──────────────────────────────┘│ 28 字节       │ 28 字节                  │
 └────────────────────────────────┴───────────────┴──────────────────────────┘
...
```
Buffer Pool 内存布局（BUFFER_POOL_SIZE = 65536 个 frame）：

frames_[0..65535]                     page_table_
┌────┬─────────────────────┐          ┌─────────────────────────────────┐
│[0] │ Page {              │          │ {fd:3, page_no:1} → frame_id 0 │
│    │   id_ = {fd:3, 1}   │──────────│ {fd:3, page_no:2} → frame_id 1 │
│    │   data_[4096]       │          │ {fd:3, page_no:3} → frame_id 2 │
│    │   is_dirty_ = false │          │ {fd:4, page_no:0} → frame_id 3 │
│    │   pin_count_ = 1    │          └─────────────────────────────────┘
│    │ }                   │
├────┼─────────────────────┤          free_list_
│[1] │ Page {              │          ┌─────────────────────────────────┐
│    │   id_ = {fd:3, 2}   │          │ frame_id: 4 → 5 → 6 → ... →    │
│    │   data_[4096]       │          │ 65535                          │
│    │   is_dirty_ = true  │          └─────────────────────────────────┘
│    │   pin_count_ = 0    │
│    │ }                   │
...
├────┼─────────────────────┤
│[3] │ Page {              │
│    │   id_ = {fd:4, 0}   │  ← 另一个文件（course.db）的文件头
│    │   ...               │
│    │ }                   │
├────┼─────────────────────┤
│[4] │ 空 Page（空闲）      │
