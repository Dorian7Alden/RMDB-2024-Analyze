...
>
> 由于 Linux 的文件系统特性，`rm` 删除的是目录项（文件名），只要进程还持有文件的 fd，文件数据就还在磁盘上，只是对外不可见了。所以**不会当场崩溃**——句柄仍然是合法的 C++ 对象，fd 也仍然有效。
>
> 但会产生三个问题：
>
> - **脏页丢失**：没先 `close_db`，缓冲池里的脏页没刷盘，表和索引的最新修改就丢了。这是最严重的后果。（脏页概念见第 1 章[存储层](../01-storage-layer/README.md)）
> - **空间泄漏**：句柄持有的 fd 一直占着已删除文件的数据。直到 fd 关闭，磁盘空间才会释放。一直不关，"幽灵文件"就一直占着空间。
> - **同名冲突**：如果之后又 `create_db` 建了一个同名数据库，`fhs_` 中残留的旧句柄仍指向已删除的旧文件，`fhs_["student"]` 拿不到新文件，逻辑混乱。
>
> 所以正确的用法是：**先 `close_db`（正常关闭句柄、落盘元数据），再 `drop_db`（删除目录）**。
