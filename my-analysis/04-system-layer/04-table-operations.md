...
这个操作是纯查询，不修改任何数据，也不申请事务级锁；它只读取内存中的 `db_.tabs_` 并格式化输出，框架中已完整实现。
...
**位置**：`src/system/sm_manager.cpp:192-217`
