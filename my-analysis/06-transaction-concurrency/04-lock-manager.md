...
死锁处理有预防和检测两大流派。RMDB 的 wait-die 属于**预防**——通过规则保证死锁永不发生。预防流派的另一种是 wound-wait（少等老、老伤少）——当老事务请求年轻事务持有的锁时，老事务"伤害"年轻事务使其中止。wound-wait 的中止次数通常少于 wait-die，但实现上需要事务能主动中止其他事务。

**检测**流派的代表是等待图（wait-for graph）——定期扫描锁表，构建"事务 A 等事务 B"的有向图，检测环。发现环后选择一个事务作为 victim 中止。等待图的优势是不无辜中止，但检测本身有 CPU 开销，且选 victim 的策略会影响公平性。大多数商业 DBMS（如 MySQL InnoDB）用等待图检测，部分也用超时（`innodb_lock_wait_timeout`）。

RMDB 选 wait-die 的原因：实现简单（只需要 `oldest_txn_id_` 比较），不引入额外的检测线程或周期性扫描开销。缺点是可能无辜中止年轻事务。
